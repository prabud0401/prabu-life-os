...
function extractBody(payload: gmail_v1.Schema$MessagePart): { text: string; html: string } {
  ...
  function processPayload(part: gmail_v1.Schema$MessagePart): void {
    if (part.body?.data) {
      ...
      if (part.mimeType === "text/plain") text = content;
      else if (part.mimeType === "text/html") html = content;
    }
    if (part.parts) {
      for (const subPart of part.parts) processPayload(subPart);
    }
  }
...
export async function readMessage(params: { messageId: string }) {
  const gmail = await getGmailClient();
  ...
  function findAttachments(part: gmail_v1.Schema$MessagePart): void {
    ...
    if (part.parts) {
      for (const subPart of part.parts) findAttachments(subPart);
    }
  }

  if (response.data.payload) findAttachments(response.data.payload);
...
export async function searchMessages(params: {
  ...
  if (result.messages.length === 0) {
    const suggestions = generateBroaderSuggestions(parseGmailQuery(params.query));
    if (suggestions) {
      return { messages, nextPageToken: result.nextPageToken, suggestions };
    }
  }
...
export async function sendDraft(params: { draftId: string }) {
  const gmail = await getGmailClient();
